...
class FriendHandlers : public QObject {
  ...
  Q_PROPERTY(QVariantList nonFriendUsers READ nonFriendUsers NOTIFY
                 nonFriendUsersChanged)
  // Property để binding danh sách yêu cầu kết bạn lên QML
  Q_PROPERTY(QVariantList friendRequests READ friendRequests NOTIFY
  ...
  Q_PROPERTY(int currentFriendStatus READ currentFriendStatus NOTIFY
                 currentFriendStatusChanged)
...
private slots:
  ...
  // Slot xử lý chung cho việc nhận tin nhắn (cả tin nhắn mới và lịch sử tin
  // nhắn)
